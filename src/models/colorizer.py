...
class Network_Prob(nn.Module):
    def __init__(self):
        """
        Initializes each part of the convolutional neural network.
        With Probability Distribution for A and B output maps.

        This network is adapted the network above from
        @online{cnn_colorization,
        author = {George Kamtziridis},
        title = {Building an Image Colorization Neural Network},
        year = {2022},
        ...
        self.t_conv1 = nn.ConvTranspose2d(256, 512-128, kernel_size=4, stride=2, padding=1)
        self.t_conv1_bn = nn.BatchNorm2d(512-128)
        self.t_conv2 = nn.ConvTranspose2d(512, 512-64, kernel_size=4, stride=2, padding=1)
        self.t_conv2_bn = nn.BatchNorm2d(512-64)
        self.t_conv3 = nn.ConvTranspose2d(512, 512-32, kernel_size=4, stride=2, padding=1)
        self.t_conv3_bn = nn.BatchNorm2d(512-32)
        self.t_conv4 = nn.ConvTranspose2d(512, 512-1, kernel_size=4, stride=2, padding=1)

        self.output = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1)
...
def get_mle(y_pred):
    ...
    y_pred = torch.argmax(y_pred, dim=1).view(batch_size, 2, height, width)-127
    return y_pred